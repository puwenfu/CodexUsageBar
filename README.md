...
- 提供常驻系统托盘入口，可把小组件放在任务栏、Codex 侧栏或仅保留托盘图标。
- 可分别调整任务栏与 Codex 侧栏中的横向位置，并在重启后保留选择。
...
左键单击小组件可刷新。右键单击小组件或托盘图标可刷新、切换仪表主题或刷新
动画、选择显示位置、调整横向位置、控制可选的开机启动、打开调试面板或退出。
点击菜单外任意位置会关闭菜单。主题、显示位置和偏移会自动保存；明暗外观由
Windows 自动控制，开机启动默认关闭。
...
目前支持位于屏幕底部的 Windows 11 主任务栏，也可对齐当前桌面版 Codex
窗口的侧栏区域。Codex 界面结构变化后可能需要重新调整侧栏偏移；首选位置
不可用时应用会回退到可用位置或系统托盘。Codex 协议变化或临时连接失败可能
延迟刷新；如有安全的历史数据，界面会继续显示最后一次成功结果。
